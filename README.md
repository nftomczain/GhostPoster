...
## Portable Windows version (.exe)

Portable Windows builds are automatically generated by GitHub Actions for every successful build.

You can download the latest portable build from the repository's **Actions** page, or build it locally:
...
The entire folder is portable — no installer is required. Simply copy or zip the folder and run `GhostPoster.exe` on any Windows 10 or Windows 11 computer.

## Portable macOS version (.app)

GhostPoster is designed to be cross-platform, with a native portable macOS build now generated automatically through GitHub Actions.

The macOS build produces a self-contained `GhostPoster.app` bundle that can be copied and run without a traditional installer.

The application can also be built locally with PyInstaller using the same project configuration.

A `.DMG` distribution package, along with macOS code signing and notarization, can be added in the future for a more polished end-user distribution experience.

If you have access to macOS hardware and would like to help test the native build, verify compatibility, or improve macOS packaging, contributions are very welcome.
...
for Linux, and a portable `GhostPoster.app` for macOS. The Windows, Linux,
and macOS application builds use PyInstaller.
